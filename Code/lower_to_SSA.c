/**
 * lower_to_SSA.c — TAC 到 SSA 的转换 (SSA Construction)
 *
 * ============================================================================
 * SSA (Static Single Assignment) 简述
 * ============================================================================
 * SSA 是一种中间表示形式，其中每个变量在程序文本中只被赋值一次。
 *
 * 例（非 SSA）：
 *   x = 1          →  x1 = 1
 *   x = x + 1      →  x2 = x1 + 1
 ...
 *
 * SSA 的核心优势：
 *   1. 数据流隐式编码在变量名中（不需要 use-def 链分析）
 *   2. 简化常量传播、死代码消除等优化
 *   3. 冲突图构造更精确
 *
 * ============================================================================
 * 算法步骤（Cytron et al., 1991）
 * ============================================================================
 *
 * Step 1: build_CFG
 *   建立控制流图——每个基本块记录其前驱和后继。
 *
 ...
 *   标准算法：对每个变量的定义，在其 DF 中的基本块处插入 phi 节点，
 *   递归处理直到不动点。
 *
 * Step 4: rename_variables
 *   重命名变量为 SSA 版本号。深度优先遍历支配树，维护每个变量的版本栈。
 *
 * ============================================================================
 */
...
/* 将普通 TAC 转换为 SSA 形式：建 CFG → 支配树 → φ 节点 → 重命名 */
